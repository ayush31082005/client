...
const OrderDetails = () => {
            ...
            <div style="font-family: 'Times New Roman', Times, serif; padding: 40px; color: #000; background: #fff; line-height: 1.4; border: 1px solid #000;">
                ...
                <div style="display: flex; justify-content: space-between; border-bottom: 2px solid #000; padding-bottom: 20px; margin-bottom: 30px;">
                    ...
                    <div style="text-align: right; flex: 1;">
                        <h2 style="margin: 0; font-size: 16px; color: #C9A84C;">SANYUKT PARIVAAR & RICH LIFE PVT LTD</h2>
                    ...
                    <div>
                        <table style="width: 100%; border-collapse: collapse;">
                            ...
                            <tr>
                                ...
                                <td style="text-transform: uppercase; color: ${orderData.paymentStatus === 'paid' ? '#C9A84C' : '#000'};">${orderData.paymentStatus || 'Pending'}</td>
                            </tr>
                        </table>
                    </div>
                    <div>
    ...
    if (loading) {
        return (
            <div className="min-h-[60vh] flex flex-col items-center justify-center p-8 bg-zinc-900 rounded-3xl animate-pulse">
                <CircularProgress sx={{ color: '#C9A84C' }} size={60} thickness={4} />
    ...
    if (!order) {
        return (
            <div className="max-w-2xl mx-auto p-10 text-center bg-zinc-900 rounded-3xl shadow-xl mt-10">
                <div className="w-24 h-24 bg-red-50 rounded-full flex items-center justify-center mx-auto mb-6">
                    <FileText className="text-red-400" size={48} />
                </div>
                <h3 className="text-2xl font-black text-zinc-100 uppercase tracking-tighter mb-2">Order Vault Missing</h3>
                <p className="text-zinc-300 mb-8">The requested order coordinates could not be found in our encrypted records.</p>
                <Button 
                    variant="contained" 
                    onClick={() => navigate('/my-account/orders')}
                    sx={{ bgcolor: '#C9A84C', px: 4, py: 1.5, borderRadius: '15px', fontWeight: 'bold' }}
                >
    ...
    return (
        ...
        >
            ...
            <div className="flex items-center justify-between mb-8">
                <button 
                    onClick={() => navigate('/my-account/orders')}
                    className="group flex items-center gap-2 px-5 py-3 bg-zinc-900 border border-yellow-900/30 rounded-2xl shadow-sm hover:shadow-md transition-all active:scale-95"
                >
                    <ChevronLeft className="group-hover:-translate-x-1 transition-transform" />
                    <span className="font-black text-xs uppercase tracking-widest text-zinc-300">Back to Hub</span>
                </button>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
                ...
                <div className="lg:col-span-7 space-y-8">
                    <div className="bg-zinc-900 rounded-[2.5rem] p-8 md:p-12 shadow-[0_30px_100px_rgba(0,0,0,0.04)] border border-slate-50 relative overflow-hidden">
                        {/* Decorative Gradient */}
                        <div className="absolute top-0 right-0 w-64 h-64 bg-yellow-50/30 rounded-full blur-3xl -mr-32 -mt-32"></div>

                        <div className="relative z-10">
                            <div className="flex items-center justify-between mb-12">
                                <div>
                                    <h1 className="text-3xl font-black text-zinc-100 tracking-tighter uppercase mb-1">Order Details</h1>
                                    <div className="flex items-center gap-2">
                                        <span className="text-[10px] font-black text-zinc-300 uppercase tracking-[0.3em]">Identity:</span>
                                        <span className="text-xs font-black text-[#C9A84C] tracking-wider">#{order._id.slice(-8).toUpperCase()}</span>
                                    </div>
                                </div>
                                <div className="text-right">
                                    <Chip 
                                        ...
                                        sx={{ 
                                            bgcolor: order.status === 'delivered' ? '#C9A84C' : order.status === 'cancelled' ? '#ef4444' : '#eab308',
                            ...
                            <div className="flex flex-col space-y-0 relative">
                                ...
                                <div className="absolute left-[23.5px] top-6 bottom-6 w-0.5 bg-zinc-900"></div>
                                    ...
                                    return (
                                        <div key={idx} className={`flex items-start gap-8 py-6 relative group transition-all`}>
                                            ...
                                            <div className={`
                                                ...
                                                ${isCompleted || isActive ? 'bg-[#C9A84C] text-zinc-100 shadow-lg shadow-yellow-200' : 'bg-zinc-900 text-slate-300 border border-yellow-900/30'}
                                                ${isActive ? 'scale-110 ring-4 ring-yellow-50' : 'scale-100'}
                                            `}>
                                                ...
                                                {idx < allPossibleSteps.length - 1 && (
                                                    <div className={`
                                                        ...
                                                        ${idx < maxReachedIndex ? 'bg-[#C9A84C]' : 'bg-zinc-800'}
                                                    `} />
                                                )}
                                                ...
                                                {isActive && idx < allPossibleSteps.length - 1 && (
                                                    <motion.div 
                                                        className="absolute left-[23.5px] top-12 w-0.5 bg-[#C9A84C] origin-top z-20"
                                                        initial={{ height: 0 }}
                                            ...
                                            <div className="flex-1 flex flex-col md:flex-row md:items-center justify-between gap-4">
                                                <div className="flex flex-col">
                                                    <h3 className={`text-base font-black uppercase tracking-tight leading-none mb-1.5 
                                                        ${isFuture ? 'text-slate-300' : 'text-zinc-100'}`}>
                                                        {step.label}
                                                    </h3>
                                                    <p className={`text-[11px] font-bold ${isFuture ? 'text-slate-200' : 'text-zinc-300'}`}>
                                                        {logEntry?.message || (isActive ? 'Current Phase' : step.description)}
                                                    </p>
                                                </div>

                                                <div className="flex flex-col items-start md:items-end">
                                                    <span className={`text-[11px] font-black uppercase tracking-widest ${isFuture ? 'text-slate-200' : 'text-zinc-100'}`}>
                                                        {logEntry ? new Date(logEntry.timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', hour12: true }) : ''}
                                                    </span>
                                                    <span className={`text-[10px] font-bold uppercase tracking-tighter ${isFuture ? 'text-slate-100' : 'text-zinc-300'}`}>
                ...
                <div className="lg:col-span-5 space-y-6">
                    {/* Item Card */}
                    <div className="bg-zinc-900/50 rounded-[2rem] p-6 border border-yellow-900/30 hover:bg-zinc-900 hover:shadow-xl transition-all duration-500 group">
                        <span className="text-[10px] font-black text-zinc-300 uppercase tracking-[0.2em] mb-4 block">Manifest Item</span>
                        <div className="flex items-center gap-6">
                            <div className="w-24 h-24 rounded-3xl bg-zinc-900 shadow-lg overflow-hidden border-4 border-white group-hover:scale-105 transition-transform">
                                {order.product?.image ? (
                                    <img 
                                        ...
                                        alt="Product"
                                    />
                                ) : <div className="w-full h-full flex items-center justify-center bg-yellow-50 text-[#C9A84C] font-black">SP</div>}
                            </div>
                            <div className="flex-1">
                                <h2 className="text-xl font-black text-zinc-100 tracking-tight leading-tight mb-1">{order.product?.name || 'Mission Supply'}</h2>
                                <p className="text-sm font-bold text-zinc-300 mb-2">Quantity: {order.quantity || 1}</p>
                                <div className="text-2xl font-black text-[#C9A84C] tracking-tighter">
                    ...
                    {/* Payment Specs */}
                    <div className="bg-zinc-900 rounded-[2rem] p-8 border border-yellow-900/30 shadow-sm">
                        <div className="flex items-center gap-3 mb-6">
                            <div className="p-2.5 bg-blue-50 text-blue-600 rounded-xl">
                                <CreditCard size={18} />
                            </div>
                            <h3 className="text-sm font-black uppercase tracking-widest text-zinc-100">Financial Spec</h3>
                        </div>
                        <div className="space-y-4">
                            <div className="flex justify-between items-center text-sm">
                                <span className="font-bold text-zinc-300 uppercase tracking-widest text-[10px]">Method</span>
                                <span className="font-black text-zinc-200">{getPaymentLabel(order.paymentMethod)}</span>
                            </div>
                            <div className="flex justify-between items-center text-sm border-b border-slate-50 pb-3 mb-3">
                                <span className="font-bold text-zinc-300 uppercase tracking-widest text-[10px]">Payment</span>
                                <span className={`font-black uppercase tracking-widest text-xs ${order.paymentStatus === 'paid' ? 'text-yellow-600' : 'text-yellow-500'}`}>
                                    {order.paymentStatus || 'Pending'}
                                </span>
                            </div>
                            <div className="flex justify-between items-center text-sm">
                                <span className="font-bold text-zinc-300 uppercase tracking-widest text-[10px]">Stage</span>
                                <div className="flex items-center gap-1.5">
                                    <div className={`w-1.5 h-1.5 rounded-full ${order.status === 'delivered' ? 'bg-yellow-500' : order.status === 'cancelled' ? 'bg-red-500' : 'bg-yellow-400'}`}></div>
                                    <span className={`font-black uppercase tracking-widest text-xs ${order.status === 'delivered' ? 'text-yellow-600' : order.status === 'cancelled' ? 'text-red-500' : 'text-yellow-500'}`}>
                            ...
                            {order.upiTransactionId && (
                                <div className="pt-4 border-t border-slate-50 flex flex-col gap-1">
                                    <span className="font-bold text-zinc-300 uppercase tracking-widest text-[10px]">Ref ID</span>
                                    <span className="font-mono text-[11px] text-zinc-300 bg-zinc-900 p-2 rounded-lg">{order.upiTransactionId}</span>
                                </div>
                            )}
                        </div>
                    </div>
                    ...
                    <div className="bg-zinc-900 rounded-[2rem] p-8 border border-yellow-900/30 shadow-sm">
                        <div className="flex items-center gap-3 mb-6">
                            ...
                            <h3 className="text-sm font-black uppercase tracking-widest text-zinc-100">Delivery Nodes</h3>
                        </div>
                        <div className="space-y-1">
                            <h4 className="font-black text-zinc-100 uppercase tracking-tight">{order.shippingInfo?.fullName}</h4>
                            <p className="text-sm text-zinc-300 font-bold leading-relaxed">{order.shippingInfo?.address}</p>
                            <p className="text-xs font-black text-zinc-300 uppercase tracking-widest mt-2">
                                {order.shippingInfo?.city}, {order.shippingInfo?.state} - {order.shippingInfo?.pincode}
                            </p>
                            <div className="pt-4 mt-4 border-t border-slate-50">
                                <p className="text-xs font-bold text-zinc-300">Secure Direct Line:</p>
                                <p className="text-sm font-black text-zinc-100 tracking-widest">{order.shippingInfo?.phone}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="flex gap-4">
                        <Button 
                            ...
                            sx={{ 
                                bgcolor: '#C9A84C', color: 'white', py: 2, 
