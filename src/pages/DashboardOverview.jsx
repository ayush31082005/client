...
const SectionHeader = ({ title, subtitle, icon: Icon }) => (
    <div className="flex items-center justify-between mb-6 mt-2">
        <div className="flex items-center gap-3">
            ...
            <div>
                <h2 className="text-[14px] font-black text-zinc-100 uppercase tracking-[0.15em] leading-none mb-1">
                    {title}
                </h2>
                {subtitle && (
        ...
        <div className="h-px flex-1 bg-zinc-800 mx-6 hidden md:block"></div>
    </div>
);
...
const ModernWalletCard = ({ title, value, color, icon: Icon, progress, showCurrency = true }) => (
    <motion.div
        ...
        className="bg-zinc-900 rounded-2xl p-5 shadow-[0_8px_30px_rgb(0,0,0,0.02)] border border-slate-50 flex flex-col relative overflow-hidden group hover:shadow-xl hover:shadow-slate-200/40 transition-all duration-300"
    >
        ...
        <div className="flex flex-col gap-1">
            <h3 className="text-[11px] font-black text-zinc-300 uppercase tracking-widest">{title}</h3>
            <div className="flex items-baseline gap-1.5">
                {showCurrency && <span className="text-xl font-black text-zinc-300">₹</span>}
                <span className="text-3xl font-black text-zinc-100 tracking-tighter">{value !== undefined ? value : "0"}</span>
            </div>
        </div>
        {progress !== undefined && (
            <div className="relative h-1.5 w-full bg-zinc-800 rounded-full mt-4 overflow-hidden">
                <motion.div
                    initial={{ width: 0 }}
...
const PerformanceMetric = ({ title, value, icon: Icon, color, trend }) => (
    <motion.div
        variants={cardVariants}
        className="bg-zinc-900 rounded-2xl p-5 border border-slate-50 shadow-sm flex items-center gap-4 group hover:border-yellow-900/30 transition-colors"
    >
        <div className={`p-3 rounded-xl ${color?.bg} ${color?.text}`}>
            {Icon && <Icon size={20} />}
        </div>
        <div className="flex flex-col flex-1">
            <span className="text-[10px] font-black text-zinc-300 uppercase tracking-widest mb-0.5">{title}</span>
...
const BusinessMatrixCard = ({ title, lValue, rValue, icon: Icon }) => (
    <motion.div
        variants={cardVariants}
        className="bg-zinc-900 rounded-2xl p-4 shadow-sm border border-slate-50 flex flex-col relative overflow-hidden group h-full"
    >
        <div className="flex items-center gap-3 mb-4">
            ...
            <h3 className="text-[10px] font-black text-zinc-300 uppercase tracking-widest leading-snug line-clamp-2 flex-1">
                {title}
            </h3>
        </div>

        <div className="grid grid-cols-2 gap-2 mt-auto">
            <div className="bg-zinc-900/80 rounded-xl p-2.5 flex flex-col gap-0.5 group-hover:bg-zinc-900 group-hover:shadow-sm transition-all border border-transparent group-hover:border-yellow-900/30">
                <span className="text-[9px] font-black text-zinc-300 uppercase tracking-widest">Left</span>
                <span className="text-xl font-black text-zinc-100 tracking-tight leading-none">{lValue !== undefined ? lValue : 0}</span>
            </div>
            <div className="bg-zinc-900/80 rounded-xl p-2.5 flex flex-col items-end gap-0.5 group-hover:bg-zinc-900 group-hover:shadow-sm transition-all border border-transparent group-hover:border-yellow-900/30">
                <span className="text-[9px] font-black text-zinc-300 uppercase tracking-widest text-right">Right</span>
                <span className="text-xl font-black text-zinc-100 tracking-tight leading-none">{rValue !== undefined ? rValue : 0}</span>
            </div>
        </div>

        <div className={`absolute bottom-0 left-0 h-1 w-full bg-[#C9A84C] opacity-40`}></div>
    </motion.div>
);
...
const recentActivities = [
    ...
    { id: 2, user: "Priya Sharma", action: "joined the network", time: "09:12 AM", date: "Today", icon: UserCheck, color: "bg-yellow-50 text-yellow-600" },
            ...
            >
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                    ...
                    <div className="lg:col-span-8 flex flex-col gap-6">
                        ...
                        <div className="mt-4 p-6 bg-[#083d1c] rounded-[2rem] text-zinc-100 flex items-center justify-between shadow-xl shadow-slate-200">
                            <div className="flex items-center gap-4">
                                <div className="p-3 bg-zinc-900/10 rounded-2xl">
                                    <ShoppingBag size={24} className="text-[#C9A84C]" />
                                </div>
                                <div className="flex flex-col">
                                    <span className="text-[11px] font-black text-yellow-200/60 uppercase tracking-widest">Total Product Purchases</span>
                                    <h4 className="text-3xl font-black tracking-tighter">₹ {Number(stats?.productPurchases || 0).toLocaleString()}</h4>
                                </div>
                            </div>
                            <button
                                onClick={() => navigate('/my-account/orders')}
                                className="px-6 py-3 bg-[#C9A84C] hover:bg-yellow-500 rounded-xl text-xs font-black uppercase tracking-widest transition-colors shadow-lg shadow-yellow-900/20"
                            >
                                View Details
                            </button>
                ...
                <div>
                    ...
                    <motion.div
                        variants={cardVariants}
                        className="bg-zinc-900 rounded-[2rem] p-4 shadow-[0_20px_60px_rgb(0,0,0,0.03)] border border-yellow-900/30 overflow-hidden relative h-[380px]"
                    >
                        {/* Gradient Masks for seamless feel */}
                        <div className="absolute top-0 left-0 w-full h-12 bg-gradient-to-b from-zinc-900 to-transparent z-10"></div>
                        <div className="absolute bottom-0 left-0 w-full h-12 bg-gradient-to-t from-zinc-900 to-transparent z-10"></div>
                        ...
                        >
                            ...
                            {[...recentActivities, ...recentActivities].map((item, index) => {
                                ...
                                return (
                                    <div
                                        key={`${item.id}-${index}`}
                                        className={`flex items-center justify-between py-5 px-6 group cursor-pointer hover:bg-zinc-900/80 transition-all border-b border-slate-50`}
                                    >
                                        <div className="flex items-center gap-5">
                                            ...
                                            <div className="flex flex-col">
                                                <div className="text-[14px] font-black text-zinc-100 leading-tight">
                                                    {item.user} <span className="font-semibold text-zinc-300">{item.action}</span>
                                                </div>
                                                <div className="text-[10px] font-black text-zinc-300 mt-1 uppercase tracking-widest group-hover:text-[#C9A84C] transition-colors">
                                        ...
                                        <div className="flex flex-col items-end gap-1">
                                            <div className="text-[11px] font-black text-zinc-100 uppercase tracking-widest">
                                                {item.time}
                                            </div>
                ...
                {/* --- FOOTER --- */}
                <div className="mt-10 pt-10 border-t border-yellow-900/30 flex flex-col md:flex-row justify-between items-center gap-6">
                    <div className="text-[10px] font-black text-zinc-300 uppercase tracking-[0.2em] flex items-center gap-8">
                        <div>System Log: <span className="text-zinc-100">Online</span></div>
                        <div className="hidden md:block w-1.5 h-1.5 rounded-full bg-zinc-700"></div>
                        <div className="hidden md:block">Update Frequency: <span className="text-zinc-100">Real-time</span></div>
                    </div>
                    <div className="flex items-center gap-4 text-[10px] font-black uppercase tracking-widest">
                        <span className="text-zinc-300">Powered by</span>
                        <span className="text-[#C9A84C] bg-yellow-50 px-3 py-1.5 rounded-lg border border-yellow-100/50">Sanyukt Executive</span>
                    </div>
                </div>
            </motion.div>
        </div>
