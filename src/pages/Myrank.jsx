...
const StatCard = ({ label, value, icon: Icon, color, isPoints }) => (
    <div className="bg-zinc-900 border border-yellow-900/30 rounded-[2rem] p-5 shadow-sm flex flex-col justify-between hover:shadow-md transition-shadow">
        <div className="flex items-center justify-between mb-4">
            <div className={`p-2.5 rounded-xl ${color} text-zinc-100 shadow-sm`}>
                <Icon className="w-5 h-5 md:w-6 md:h-6" />
            </div>
        </div>
        <div>
            <p className="text-xl md:text-2xl font-black text-zinc-100 tracking-tighter leading-none mb-1 uppercase">
                {value}{isPoints && <span className="text-[10px] ml-1 text-zinc-300 uppercase">PV</span>}
            </p>
            <p className="text-[9px] md:text-[10px] font-bold text-zinc-300 uppercase tracking-widest">{label}</p>
        </div>
    </div>
);
    ...
    if (loading) return (
        <div className="min-h-screen flex items-center justify-center bg-[#F8FAFC]">
            <div className="text-center">
                <div className="w-14 h-14 border-4 border-yellow-900/30 border-t-emerald-500 rounded-full mb-6 mx-auto animate-spin" />
                <p className="text-zinc-300 font-bold uppercase text-[10px] tracking-[0.2em]">Analyzing Achievements...</p>
            </div>
        </div>
    );
        ...
        <div className="min-h-screen bg-[#F8FAFC] pb-20 relative overflow-hidden">
            ...
            <div className="relative z-10 p-4 md:p-8 max-w-5xl mx-auto">
                ...
                <div className="flex items-center gap-6 mb-8 md:mb-10">
                    <button
                        onClick={() => navigate(-1)}
                        className="w-12 h-12 rounded-2xl bg-zinc-900 flex items-center justify-center shadow-lg shadow-slate-200/50 text-zinc-300 hover:text-zinc-300 transition"
                    >
                        <ArrowLeft className="w-5 h-5" />
                    </button>
                    <div>
                        <h1 className="text-3xl font-black text-zinc-100 uppercase tracking-tighter leading-none mb-1">My Rank</h1>
                        <p className="text-[10px] text-zinc-300 font-black uppercase tracking-[0.2em]">Lifetime Achievement Journey</p>
                    </div>
                </div>

                {/* ── Current Rank Hero Card ── */}
                <div
                    className={`${currentRank.color} rounded-[2rem] md:rounded-[2.5rem] p-6 md:p-10 text-zinc-100 shadow-2xl mb-10 border border-black/5 relative overflow-hidden`}
                >
                    <div className="relative z-10">
                        ...
                        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-6 md:mb-10 border-b border-white/10 pb-6 md:pb-10">
                            <div className="flex items-center gap-4 md:gap-6">
                                <div className="w-16 h-16 md:w-20 md:h-20 bg-zinc-900 shadow-xl rounded-[1.5rem] md:rounded-[2rem] flex items-center justify-center shrink-0">
                                    <Trophy className="w-8 h-8 md:w-10 md:h-10 text-zinc-100" />
                                </div>
                                <div className="min-w-0">
                                    <p className="text-[10px] md:text-[11px] font-black text-zinc-100/80 uppercase tracking-widest mb-1">Current Achievement</p>
                            ...
                            <div className="bg-black/10 px-6 py-4 rounded-2xl md:rounded-3xl border border-white/5">
                                <p className="text-[10px] md:text-[11px] font-black text-zinc-100/70 uppercase tracking-widest mb-1">Total Matched PV</p>
                        ...
                        {nextRank ? (
                            <div className="bg-black/10 p-5 md:p-6 rounded-[1.75rem] md:rounded-[2rem] border border-white/5">
                                <div className="flex justify-between items-end mb-4">
                                    <div className="min-w-0">
                                        <p className="text-[10px] md:text-[11px] font-black text-zinc-100/70 uppercase tracking-widest mb-2">Next Target: {nextRank.name}</p>
                                    ...
                                    <div className="text-right shrink-0 ml-4">
                                        <p className="text-2xl font-black text-zinc-100 tabular-nums leading-none">{(progressPct || 0).toFixed(1)}%</p>
                                        <p className="text-[10px] font-black text-zinc-100/70 uppercase tracking-widest mt-1">Progress</p>
                                    </div>
                                </div>
                                <div className="h-3 md:h-4 w-full bg-zinc-900/10 rounded-full overflow-hidden border border-white/5">
                                    <div
                        ...
                        <div className="mt-6 flex items-center gap-2.5 px-4 py-3 bg-black/20 rounded-xl w-fit">
                            <Info className="w-4 h-4 text-zinc-100/80 shrink-0" />
                            <p className="text-[9px] font-black uppercase tracking-[0.05em] leading-tight text-zinc-100/90">Equal matching PV required on Left & Right legs</p>
                        </div>
                    </div>
                </div>
                ...
                <div className="bg-zinc-900 rounded-[2rem] md:rounded-[2.5rem] border border-yellow-900/30 shadow-2xl shadow-slate-200/50 overflow-hidden">
                    <div className="p-6 md:p-8 border-b border-slate-50 flex flex-col md:flex-row md:items-center justify-between gap-4">
                        <div className="flex items-center gap-3">
                            <div className="w-12 h-12 bg-slate-900 rounded-2xl flex items-center justify-center text-zinc-100 shrink-0">
                                <Award className="w-6 h-6" />
                            </div>
                            <div>
                                <h3 className="text-lg font-black text-zinc-100 uppercase tracking-tight">The Rank Journey</h3>
                                <p className="text-[10px] text-zinc-300 font-bold uppercase tracking-widest">Achieve milestones, unlock rewards</p>
                            </div>
                        </div>
                        <div className="flex items-center gap-2 px-4 py-2 bg-zinc-900 rounded-xl border border-slate-50 self-start md:self-auto">
                            <Activity className="w-3.5 h-3.5 text-zinc-300" />
                            <span className="text-[9px] font-black text-zinc-300 uppercase tracking-widest">Unlock Status</span>
                        </div>
                    </div>
                        ...
                        <div className="space-y-4">
                            {RANKS.filter(rank => matchedPV < rank.matchPV).map((rank) => {
                                ...
                                return (
                                    <div
                                        key={rank.name}
                                        className={`flex items-center gap-4 p-5 rounded-[1.75rem] border transition-all duration-300 ${isNext ? 'bg-amber-50/80 border-amber-200 ring-4 ring-amber-100/50 shadow-xl' : 'bg-zinc-900 border-yellow-900/30 shadow-sm'}`}
                                    >
                                        <div className="w-12 h-12 md:w-14 md:h-14 shrink-0 rounded-xl md:rounded-2xl flex items-center justify-center bg-zinc-800 text-zinc-300">
                                            <Lock className="w-4 h-4 md:w-5 md:h-5 text-slate-300" />
                                        </div>

                                        <div className="flex-1 min-w-0">
                                            <div className="flex flex-wrap items-center gap-2 mb-1">
                                                <h4 className="text-lg md:text-xl font-black tracking-tight text-zinc-100 uppercase">
                                                    {rank.name}
                                                </h4>
                                                {isNext && (
                                                    <span className="px-3 py-1 bg-slate-900 text-zinc-100 text-[8px] font-black uppercase tracking-widest rounded-full">
                                                        Target
                                                    </span>
                                                )}
                                            </div>
                                            <div className="flex items-center gap-1.5">
                                                ...
                                                <p className="text-[10px] md:text-[11px] font-black text-zinc-300 uppercase tracking-widest leading-none">Req: {rank.matchPV.toLocaleString()} PV</p>
                                            </div>
                                        </div>

                                        <div className="text-right flex flex-col items-end gap-1">
                                            <p className="text-[8px] font-black text-zinc-300 uppercase tracking-[0.2em]">Benefit</p>
                                            <div className="px-5 py-2 rounded-xl text-xs font-black tracking-tight uppercase bg-zinc-800 text-zinc-200 border border-yellow-900/30">
                                                {rank.reward}
                                            </div>
                                        </div>
                                    </div>
