        ...
        <div className="p-4 md:p-6 max-w-7xl mx-auto bg-zinc-900 min-h-screen">
            ...
            <div id="page-header" className="flex flex-col sm:flex-row sm:items-center gap-4 mb-6 opacity-0">
                <button
                    ...
                    className="p-2 hover:bg-zinc-800 rounded-lg transition-all duration-300 hover:scale-110 w-fit group active:scale-95"
                >
                    <ArrowLeft className="w-5 h-5 text-zinc-300 group-hover:-translate-x-1 transition-transform duration-300" />
                </button>
                <div>
                    <h1 className="text-2xl md:text-3xl font-black text-zinc-100">Repurchase Level Income</h1>
                    <p className="text-sm text-zinc-300 mt-1">Multi-level earnings from team repurchases</p>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
                ...
                <div
                    ...
                    className="stats-card bg-zinc-900 p-5 rounded-xl border border-yellow-900/30 shadow-sm hover:shadow-lg transition-all duration-300 hover:-translate-y-1 opacity-0"
                >
                    <div className="flex items-center justify-between mb-3">
                        <TrendingUp className="w-8 h-8 text-yellow-600 icon-bounce" />
                ...
                >
                    <div className="flex items-center justify-between mb-3">
                        <Users className="w-8 h-8 text-yellow-600 icon-pulse" />
                ...
                >
                    <div className="flex items-center justify-between mb-3">
                        <Layers className="w-8 h-8 text-yellow-600 icon-pulse" />
            ...
            {generationBreakdown.length > 0 && (
                <div className="mb-6">
                    <h2 className="text-base font-black text-zinc-200 mb-3 px-1">Generation Breakdown</h2>
                            ...
                            >
                                ...
                                <div className="flex items-center justify-between mb-3">
                                    <div className="flex items-center gap-2">
                                        <span className="w-8 h-8 rounded-full bg-yellow-100 text-yellow-700 text-xs font-black flex items-center justify-center border border-yellow-200">
                                            L{gen.generation}
                                        </span>
                                        <span className="text-sm font-bold text-zinc-200">Level {gen.generation}</span>
                                    </div>
                                ...
                                {/* Commission % */}
                                <p className="text-xs text-zinc-300 mb-3">{gen.commissionPercent}% Commission</p>

                                {/* Stats row */}
                                <div className="flex justify-between text-xs text-zinc-300 mb-2">
                                    <span>Downline Members</span>
                                    <span className="font-bold text-zinc-200">{gen.downlineMembers}</span>
                                </div>
                                <div className="flex justify-between text-xs text-zinc-300 mb-3">
                                    <span>Business Volume</span>
                                    <span className="font-bold text-zinc-200">₹{gen.totalBV.toLocaleString('en-IN')}</span>
                                </div>
                                ...
                                <p className="text-right text-xs text-yellow-600 font-bold mt-1">{gen.commissionPercent}%</p>
                            </div>
                        ))}
                    </div>
                </div>
            ...
            <div
                ref={tableRef}
                className="bg-zinc-900 rounded-xl border border-yellow-900/30 overflow-hidden shadow-sm opacity-0"
            >
                <div className="px-5 py-4 border-b border-yellow-900/30">
                    <h2 className="text-base font-black text-zinc-200">Recent Transactions</h2>
                </div>
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full min-w-[560px]">
                            <thead className="bg-zinc-900 border-b border-yellow-900/30">
                                <tr>
                                    <th className="px-5 py-3 text-left text-xs font-black text-zinc-300 uppercase tracking-wider">Date</th>
                                    <th className="px-5 py-3 text-left text-xs font-black text-zinc-300 uppercase tracking-wider">From Member</th>
                                    <th className="px-5 py-3 text-left text-xs font-black text-zinc-300 uppercase tracking-wider">Generation</th>
                                    <th className="px-5 py-3 text-left text-xs font-black text-zinc-300 uppercase tracking-wider">BV</th>
                                    <th className="px-5 py-3 text-left text-xs font-black text-zinc-300 uppercase tracking-wider">Commission</th>
                                    <th className="px-5 py-3 text-left text-xs font-black text-zinc-300 uppercase tracking-wider">Amount</th>
                                    <th className="px-5 py-3 text-left text-xs font-black text-zinc-300 uppercase tracking-wider">Status</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-gray-100">
                                {recentTransactions.map((tx, index) => (
                                    <tr
                                        ...
                                        className="table-row hover:bg-zinc-900 transition-all duration-300 cursor-pointer opacity-0"
                                    >
                                        <td className="px-5 py-3 text-sm text-zinc-300 whitespace-nowrap">
                                            {new Date(tx.createdAt).toLocaleDateString('en-IN')}
                                        </td>
                                        <td className="px-5 py-3 text-sm font-medium text-zinc-100 whitespace-nowrap">
                                        ...
                                        <td className="px-5 py-3 text-sm font-bold text-yellow-600 whitespace-nowrap">
