...
const AnimatedCard = styled(Card)(() => ({
    ...
    '&:hover': {
        ...
        boxShadow: '0 8px 20px rgba(201,168,76,0.15)',
    },
}));
...
const StatusChip = styled(Chip)(({ status }) => ({
    ...
    ...(status === 'Pending' && { backgroundColor: 'rgba(201,168,76,0.15)', color: '#C9A84C', border: '1px solid rgba(247, 147, 30, 0.3)' }),
    ...(status === 'In Progress' && { backgroundColor: 'rgba(201,168,76,0.15)', color: '#C9A84C', border: '1px solid rgba(10, 122, 47, 0.3)' }),
...
const MyAccount = ({ defaultTab }) => {
    ...
    const renderKycForm = () => (
        <Box>
            <Paper variant="outlined" sx={{ borderRadius: '14px', p: 2.5, mb: 4, bgcolor: '#1A1A1A', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <Box>
                ...
                <StatusChip label={userData.kycStatus || 'Pending'} status={userData.kycStatus || 'Pending'}
                    sx={{ px: 2, py: 2.5, fontSize: '0.9rem', borderRadius: '8px', ...(userData.kycStatus === 'Verified' && { bgcolor: '#C9A84C', color: 'white' }), ...(userData.kycStatus === 'Rejected' && { bgcolor: '#d32f2f', color: 'white' }), ...(userData.kycStatus === 'Submitted' && { bgcolor: '#C9A84C', color: 'white' }), ...(!userData.kycStatus && { bgcolor: '#757575', color: 'white' }) }} />
            </Paper>
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', mb: 1.5, gap: 1 }}>
                ...
                <Chip label="JPG, PNG" size="small" sx={{ height: 20, fontSize: '10px', fontWeight: 600, bgcolor: '#e8f5e9', color: '#C9A84C' }} />
            </Box>
            <Paper variant="outlined" sx={{ borderRadius: '12px', p: 3, mb: 4 }}>
                <Grid container spacing={3}>
                    {['aadharFront', 'aadharBack', 'panCard', 'passbook'].map((docType) => (
                        <Grid item xs={12} sm={6} md={3} key={docType}>
                            <Box sx={{ border: '2px dashed rgba(201,168,76,0.2)', borderRadius: '12px', p: 2, textAlign: 'center', position: 'relative', height: '160px', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', bgcolor: kycData.kycDocuments[docType] ? '#fff' : '#1A1A1A', transition: 'all 0.2s', '&:hover': { borderColor: '#C9A84C', bgcolor: '#f4fbf5' } }}>
                                {kycData.kycDocuments[docType] ? (
                                    <>
                                        ...
                                        {!kycReadOnly && (
                                            <IconButton size="small" sx={{ position: 'absolute', top: -8, right: -8, bgcolor: '#F5F5F5', border: '1px solid #eee', '&:hover': { bgcolor: '#ffebee' } }}
                                    ...
                                    <>
                                        ...
                                        {!kycReadOnly && (
                                            <Button component="label" size="small" variant="outlined" sx={{ textTransform: 'none', borderRadius: '6px', px: 2, color: '#C9A84C', borderColor: '#C9A84C' }}>
                                                Upload
            ...
            {!kycReadOnly && (
                <Box sx={{ display: 'flex', justifyContent: 'flex-end' }}>
                    <Button variant="contained" size="large" onClick={handleKycSubmit} disabled={kycSubmitting}
                        ...
                        sx={{ bgcolor: '#C9A84C', '&:hover': { bgcolor: '#085c22' }, py: 1.5, px: 4, borderRadius: '8px', fontSize: '15px', fontWeight: 600 }}>
            ...
            <FullPageContainer sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
                <Zoom in={true} timeout={1000}>
                    <Box sx={{ textAlign: 'center' }}>
                        <CircularProgress size={isMobile ? 50 : 60} thickness={4} sx={{ color: '#C9A84C' }} />
                        <Typography variant={isMobile ? "body1" : "h6"} sx={{ mt: 2, color: '#C9A84C', fontWeight: 500 }}>Loading your profile...</Typography>
                    </Box>
                </Zoom>
            </FullPageContainer>
        );
            ...
            <FullPageContainer sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
                <Fade in={true} timeout={1000}>
                    <Paper sx={{ p: isMobile ? 3 : 4, textAlign: 'center', borderRadius: '16px', maxWidth: '500px', width: '100%', margin: '0 auto' }}>
                        ...
                        <Button variant="contained" sx={{ mt: 2, bgcolor: '#C9A84C' }} onClick={() => navigate('/login')}>Go to Login</Button>
                    </Paper>
                </Fade>
            </FullPageContainer>
        );
        ...
        <FullPageContainer>
            <Container maxWidth="lg" disableGutters sx={{ px: { xs: 1, sm: 2, md: 0 }, height: '100%', display: 'flex', flexDirection: 'column' }}>
                ...
                <Slide direction="down" in={showContent} timeout={800}>
                    <AnimatedPaper sx={{
                        p: { xs: 3, sm: 5 }, mb: 4,
                        background: 'linear-gradient(135deg, #C9A84C 0%, #1a8c3a 50%, #065a22 100%)',
                        ...
                        <Box sx={{ display: 'flex', flexDirection: { xs: 'column', sm: 'row' }, justifyContent: 'space-between', alignItems: { xs: 'flex-start', sm: 'center' }, gap: 2 }}>
                            <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                                <Avatar src={profileImage || undefined} sx={{ bgcolor: '#C9A84C', width: { xs: 64, sm: 80 }, height: { xs: 64, sm: 80 }, fontSize: 28, fontWeight: 800, border: '4px solid rgba(255,255,255,0.4)', '&:hover': { transform: 'scale(1.1) rotate(5deg)' }, transition: 'transform 0.3s' }}>
                ...
                {isMobile && defaultTab === undefined && (
                    <Box sx={{ display: 'flex', mb: 1, px: 1 }}>
                        <Button variant="contained" startIcon={<MenuIcon />} onClick={(e) => setMenuAnchorEl(e.currentTarget)}
                            sx={{ bgcolor: '#C9A84C', borderRadius: '12px', textTransform: 'none', fontWeight: 600, '&:hover': { bgcolor: '#1a8c3a' } }}>
                            Menu
                        </Button>
                    </Box>
                )}
                    ...
                    {navItems.map((item) => (
                        <MenuItem key={item.index} onClick={() => { navigate(item.path); setMenuAnchorEl(null); }}
                            ...
                            <ListItemIcon sx={{ color: 'inherit', minWidth: 32 }}>
                                {React.cloneElement(item.icon, { sx: { fontSize: 18, color: tabValue === item.index ? '#C9A84C' : 'rgba(255,255,255,0.8)' } })}
                            </ListItemIcon>
                ...
                <Fade in={showContent} timeout={800}>
                    <AnimatedPaper sx={{ borderRadius: defaultTab !== undefined ? '0px' : '16px', overflow: 'hidden', display: 'flex', flex: 1, minHeight: 0, boxShadow: defaultTab !== undefined ? 'none' : undefined, border: defaultTab !== undefined ? 'none' : undefined, bgcolor: defaultTab !== undefined ? 'transparent' : undefined }}>
                        {/* Left Sidebar */}
                        <Box sx={{ width: { xs: 0, sm: '220px' }, display: defaultTab !== undefined ? 'none' : { xs: 'none', sm: 'flex' }, flexShrink: 0, background: 'linear-gradient(180deg, #C9A84C 0%, #1a8c3a 60%, #C9A84C 100%)', flexDirection: 'column', py: 3, overflowY: 'auto', '&::-webkit-scrollbar': { width: '4px' }, '&::-webkit-scrollbar-thumb': { backgroundColor: 'rgba(255,255,255,0.2)', borderRadius: '10px' } }}>
                            {navItems.map((item) => (
                                <Box key={item.index} onClick={() => navigate(item.path)} sx={{ display: 'flex', alignItems: 'center', gap: { xs: 0, sm: 1.5 }, justifyContent: { xs: 'center', sm: 'flex-start' }, px: { xs: 0, sm: 2.5 }, py: 1.75, mx: 1.5, mb: 0.5, borderRadius: '10px', cursor: 'pointer', transition: 'all 0.2s ease', bgcolor: tabValue === item.index ? 'rgba(255,255,255,0.2)' : 'transparent', borderLeft: tabValue === item.index ? '3px solid #C9A84C' : '3px solid transparent', '&:hover': { bgcolor: 'rgba(255,255,255,0.15)' } }}>
                                    <Box sx={{ color: tabValue === item.index ? '#C9A84C' : 'rgba(255,255,255,0.8)', display: 'flex' }}>
                        ...
                        {/* Right Content Panel */}
                        <Box sx={{ flex: 1, p: defaultTab !== undefined ? { xs: 0, sm: 1, md: 2 } : { xs: 1.5, sm: 2, md: 3 }, overflowY: 'auto', bgcolor: defaultTab !== undefined ? 'transparent' : '#1A1A1A', height: '100%', position: 'relative' }}>
                            ...
                            {tabValue === 0 && (
                                <Box>
                                    <Box sx={{ mb: 3, borderRadius: '18px', overflow: 'hidden', border: '1px solid #e8f5e9', boxShadow: '0 4px 20px rgba(10,122,47,0.1)' }}>
                                        <Box sx={{ height: '6px', background: 'linear-gradient(90deg, #C9A84C, #C9A84C, #C9A84C)' }} />
                                        <Box sx={{ p: { xs: 2, sm: 3, md: 4 }, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 4, bgcolor: '#F5F5F5', flexWrap: 'wrap' }}>
                                            <Box sx={{ display: 'flex', alignItems: { xs: 'flex-start', sm: 'center' }, gap: 2.5, flexDirection: { xs: 'column', sm: 'row' }, width: { xs: '100%', sm: 'auto' } }}>
                                                <Box sx={{ position: 'relative', display: 'inline-flex' }}>
                                                    <Avatar src={profileImage || undefined} sx={{ width: 80, height: 80, bgcolor: '#C9A84C', fontSize: 32, fontWeight: 700, border: '3px solid #e8f5e9' }}>
                                                        {!profileImage && (userData.userName || 'U')[0].toUpperCase()}
                                                    </Avatar>
                                                    {editMode && (
                                                        <Tooltip title="Upload photo">
                                                            <Box component="label" htmlFor="profile-image-upload" sx={{ position: 'absolute', bottom: 0, right: 0, bgcolor: '#C9A84C', borderRadius: '50%', width: 26, height: 26, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', border: '2px solid white', '&:hover': { bgcolor: '#085c22' } }}>
                                                ...
                                                <Box>
                                                    ...
                                                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mt: 0.75 }}>
                                                        <Box sx={{ bgcolor: '#e8f5e9', color: '#C9A84C', fontWeight: 600, fontSize: '11px', px: 1.5, py: 0.25, borderRadius: '20px' }}>ID: {formatValue(userData.memberId)}</Box>
                                                        <Box sx={{ bgcolor: '#fffbed', color: '#C9A84C', fontWeight: 600, fontSize: '11px', px: 1.5, py: 0.25, borderRadius: '20px' }}>{formatValue(userData.position)}</Box>
                                                    </Box>
                                                    <Typography sx={{ color: '#666', fontSize: '13px', mt: 0.5 }}>{formatValue(userData.email)}</Typography>
                                                </Box>
                                            </Box>
                                            {!editMode ? (
                                                <Button variant="outlined" startIcon={<EditIcon />} onClick={handleEditStart} size="small" sx={{ borderColor: '#C9A84C', color: '#C9A84C', borderRadius: '8px', fontWeight: 600, '&:hover': { bgcolor: '#e8f5e9' }, ml: { xs: 0, sm: 'auto' } }}>Edit Profile</Button>
                                            ) : (
                                                <Box sx={{ display: 'flex', gap: 1, ml: { xs: 0, sm: 'auto' } }}>
                                                    <Button variant="contained" startIcon={saving ? <CircularProgress size={14} sx={{ color: 'white' }} /> : <SaveIcon />} onClick={handleSaveProfile} disabled={saving} size="small" sx={{ bgcolor: '#C9A84C', borderRadius: '8px', fontWeight: 600, '&:hover': { bgcolor: '#085c22' } }}>Save</Button>
                                                        ...
                                                        <Grid item xs={12} sm={6} key={fi}>
                                                            <Box sx={{ mb: 1 }}>
                                                                <Typography sx={{ color: '#444', fontSize: '11px', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '1px', mb: 1.2, display: 'flex', alignItems: 'center', gap: 1 }}>
                                                                    {React.cloneElement(field.icon, { sx: { fontSize: 16, color: '#C9A84C' } })} {field.label}
                                                                </Typography>
                                                                <Box sx={{
                                                                    px: 2.5,
                                                                    py: 1.8,
                                    ...
                                    <Paper variant="outlined" sx={{ borderRadius: '18px', overflow: 'hidden', mb: 3 }}>
                                        <Box sx={{ px: 3, py: 2, bgcolor: '#f8fbf9', borderBottom: '1px solid #f0f0f0' }}>
                                            <Typography sx={{ fontWeight: 800, fontSize: '16px', color: '#111', display: 'flex', alignItems: 'center', gap: 1.2 }}>
                                                <FingerprintIcon sx={{ fontSize: 22, color: '#C9A84C' }} /> Account Details
                                            </Typography>
                                        </Box>
                                        <Box sx={{ px: 3, py: 1.5 }}>
                                            <Grid container spacing={4} sx={{ py: 3, px: 1 }}>
                                                {[
                                                    { label: 'Self Sponsor ID', value: formatValue(userData.memberId), icon: <FingerprintIcon />, accent: '#C9A84C' },
                                                    { label: 'Referrer Member ID', value: formatValue(userData.sponsorId), icon: <GroupsIcon />, accent: '#C9A84C' },
                                                    { label: 'Sponsor Full Name', value: formatValue(userData.sponsorName), icon: <PersonIcon />, accent: '#C9A84C' },
                                                    { label: 'Registered State', value: formatValue(userData.state), icon: <FlagIcon />, accent: '#C9A84C' },
                                                ].map((field, fi) => (
                                                    <Grid item xs={12} sm={6} key={fi}>
                                                        <Box sx={{ mb: 1 }}>
                                                            <Typography sx={{ color: '#444', fontSize: '11px', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '1px', mb: 1.2, display: 'flex', alignItems: 'center', gap: 1 }}>
                                    ...
                                    {/* KYC in Profile */}
                                    <Box sx={{ mb: 3 }}>
                                        <Typography variant="h6" sx={{ color: '#C9A84C', mb: 2, fontWeight: 700, fontSize: '16px', display: 'flex', alignItems: 'center', gap: 1 }}>
                            ...
                            {tabValue === 1 && (
                                <Box>
                                    <Box sx={{ mb: 3, borderRadius: '18px', overflow: 'hidden', border: '1px solid #e8f5e9', boxShadow: '0 4px 20px rgba(10,122,47,0.1)' }}>
                                        <Box sx={{ height: '6px', background: 'linear-gradient(90deg, #C9A84C, #C9A84C, #C9A84C)' }} />
                                        <Box sx={{ p: { xs: 2, sm: 3, md: 4 }, bgcolor: '#F5F5F5', display: 'flex', alignItems: 'center', gap: 3 }}>
                                            <Box sx={{ width: { xs: 40, sm: 48 }, height: { xs: 40, sm: 48 }, borderRadius: '12px', bgcolor: '#e8f5e9', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                                                <LocationOnIcon sx={{ color: '#C9A84C', fontSize: { xs: 20, sm: 26 } }} />
                                            </Box>
                                            <Box>
                                    ...
                                    <Paper variant="outlined" sx={{ borderRadius: '18px', overflow: 'hidden', mb: 3 }}>
                                        <Box sx={{ px: 3, py: 2, bgcolor: '#f8fbf9', borderBottom: '1px solid #f0f0f0' }}>
                                            <Typography sx={{ fontWeight: 800, fontSize: '16px', color: '#111', display: 'flex', alignItems: 'center', gap: 1.2 }}>
                                                <HomeIcon sx={{ fontSize: 22, color: '#C9A84C' }} /> Location Details
                                            </Typography>
                                        </Box>
                                        <Box sx={{ px: 3, py: 1.5 }}>
                                            <Grid container spacing={2.5} sx={{ py: 2 }}>
                                                {[
                                                    { label: 'State', value: formatValue(userData.state), icon: <FlagIcon sx={{ fontSize: 18, color: '#C9A84C' }} /> },
                                                    { label: 'District', value: formatValue(userData.district), icon: <LocationOnIcon sx={{ fontSize: 18, color: '#C9A84C' }} /> },
                                                    { label: 'Assembly Area', value: formatValue(userData.assemblyArea), icon: <GroupsIcon sx={{ fontSize: 18, color: '#C9A84C' }} /> },
                                                    { label: 'Block', value: formatValue(userData.block), icon: <VillaIcon sx={{ fontSize: 18, color: '#C9A84C' }} /> },
                                                    { label: 'Village Council', value: formatValue(userData.villageCouncil), icon: <AccountCircleIcon sx={{ fontSize: 18, color: '#C9A84C' }} /> },
                                                    { label: 'Village', value: formatValue(userData.village), icon: <AgricultureIcon sx={{ fontSize: 18, color: '#C9A84C' }} /> },
                                                ].map((field, fi) => (
                                                    <Grid item xs={12} key={fi}>
                                                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, p: 2, borderRadius: '12px', bgcolor: '#fcfdfc', border: '1px solid rgba(201,168,76,0.1)', transition: 'all 0.2s', '&:hover': { transform: 'translateY(-2px)', boxShadow: '0 4px 12px rgba(201,168,76,0.1)', bgcolor: '#F5F5F5' } }}>
                                    ...
                                    <Paper variant="outlined" sx={{ borderRadius: '16px', overflow: 'hidden' }}>
                                        <Box sx={{ px: 2, py: 1.5, bgcolor: '#fffbed', borderBottom: '1px solid #f0f0f0' }}>
                                            <Typography sx={{ fontWeight: 700, fontSize: '15px', color: '#111', display: 'flex', alignItems: 'center', gap: 1 }}>
                                                <LocationOnIcon sx={{ fontSize: 20, color: '#C9A84C' }} /> Shipping Address
                                            </Typography>
                                        </Box>
                                        <Box sx={{ px: 2, py: 1.5, bgcolor: '#F5F5F5' }}>
                                            <Box sx={{ display: 'flex', alignItems: 'flex-start', gap: 2 }}>
                                                <Box sx={{ bgcolor: '#fff8e1', p: 1.5, borderRadius: '10px', display: 'flex' }}><HomeIcon sx={{ color: '#C9A84C' }} /></Box>
                                                <Box>
                            ...
                            {/* ── Orders ── */}
                            {tabValue === 2 && (
                                <Box sx={{ bgcolor: '#F5F5F5', borderRadius: '12px', p: 3, boxShadow: '0 2px 12px rgba(0,0,0,0.5)' }}>
                                    ...
                                    <Box sx={{ borderBottom: 1, borderColor: '#f0f0f0', mb: 4 }}>
                                        <Tabs value={orderTab} onChange={(e, v) => setOrderTab(v)} variant="scrollable" scrollButtons="auto"
                                            sx={{ minHeight: '44px', '& .MuiTab-root': { textTransform: 'none', fontWeight: 700, fontSize: '14px', minWidth: 'auto', px: 3, py: 1, minHeight: '44px', color: '#666' }, '& .Mui-selected': { color: '#C9A84C !important' }, '& .MuiTabs-indicator': { backgroundColor: '#C9A84C', height: 3, borderRadius: '3px 3px 0 0' } }}>
                                    ...
                                    {ordersLoading ? (
                                        <Box sx={{ display: 'flex', justifyContent: 'center', py: 8 }}><CircularProgress size={40} sx={{ color: '#C9A84C' }} /></Box>
                                    ) : (
                                        <Box>
                                            {userOrders.length === 0 && (
                                                    ...
                                                    <>
                                                                        ...
                                                                        {filteredOrders.map((order, index) => {
                                                                            ...
                                                                            return (
                                                                                <TableRow key={order._id || index} hover sx={{ bgcolor: index % 2 === 0 ? '#fcfdfc' : 'white', '&:hover': { bgcolor: '#f4faf5' } }}>
                                                                                    ...
                                                                                    <TableCell sx={{ fontWeight: 800, color: '#C9A84C', fontSize: '14px', borderBottom: 'none' }}>₹{parseFloat(total).toFixed(2)}</TableCell>
                                                                                    <TableCell sx={{ color: '#666', fontSize: '13px', borderBottom: 'none' }}>{date}</TableCell>
                                                                                    <TableCell align="center" sx={{ borderBottom: 'none' }}>
                                                                                        <Box sx={{ display: 'flex', gap: 1, justifyContent: 'center' }}>
                                                                                            <Button size="small" variant="outlined" sx={{ textTransform: 'none', borderRadius: '6px', borderColor: '#C9A84C', color: '#C9A84C', fontWeight: 600 }} onClick={() => navigate(`/order-details/${order._id}`)}>Details</Button>
                                                        ...
                                                        <Box sx={{ display: { xs: 'flex', md: 'none' }, flexDirection: 'column', gap: 2 }}>
                                                            {filteredOrders.map((order, index) => {
                                                                ...
                                                                const statusMatch = order.status?.toLowerCase() || 'pending';
                                                                return (
                                                                    <Paper key={order._id || index} variant="outlined" sx={{ p: 2, borderRadius: '12px', bgcolor: '#F5F5F5' }}>
                                                                        ...
                                                                        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                                                            <Typography sx={{ fontWeight: 800, color: '#C9A84C', fontSize: '15px' }}>₹{parseFloat(total).toFixed(2)}</Typography>
                                                                            <Box sx={{ display: 'flex', gap: 1 }}>
                                                                                <Button size="small" variant="text" sx={{ color: '#C9A84C', fontWeight: 700, minWidth: 'auto', p: 0.5 }} onClick={() => navigate(`/order-details/${order._id}`)}>Details</Button>
                                                                                {(statusMatch === 'shipped' || statusMatch === 'delivered') && (
                            ...
                            {tabValue === 3 && (
                                <Box>
                                    <Typography variant="h6" sx={{ color: '#C9A84C', mb: 3, fontWeight: 700, borderBottom: '3px solid #C9A84C', pb: 1, display: 'inline-block' }}>
                                        Transaction History
                                    </Typography>
                                    ...
                                    <Grid container spacing={2} sx={{ mb: 4 }}>
                                        <Grid item xs={6} sm={3}>
                                            <Paper sx={{ p: 2, borderRadius: '12px', borderLeft: '4px solid #C9A84C', bgcolor: '#F5F5F5', boxShadow: '0 2px 8px rgba(0,0,0,0.5)' }}>
                                        ...
                                        <Grid item xs={6} sm={3}>
                                            <Paper sx={{ p: 2, borderRadius: '12px', borderLeft: '4px solid #e91e63', bgcolor: '#F5F5F5', boxShadow: '0 2px 8px rgba(0,0,0,0.5)' }}>
                                                <Typography variant="caption" sx={{ color: '#666', fontWeight: 600, textTransform: 'uppercase', fontSize: '10px' }}>Donations</Typography>
                                                <Typography variant="h6" sx={{ fontWeight: 800, mt: 0.5, color: '#111' }}>{userTransactions.filter(t => t.type === 'donation').length}</Typography>
                                            </Paper>
                                        </Grid>
                                        <Grid item xs={6} sm={3}>
                                            <Paper sx={{ p: 2, borderRadius: '12px', borderLeft: '4px solid #2196f3', bgcolor: '#F5F5F5', boxShadow: '0 2px 8px rgba(0,0,0,0.5)' }}>
                                    ...
                                    <Box sx={{ mb: 2, display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 2 }}>
                                        <Typography variant="h6" sx={{ color: '#C9A84C', fontWeight: 700, fontSize: '16px', display: 'flex', alignItems: 'center', gap: 1 }}>
                                                ...
                                                <Button key={f.key} size="small"
                                                    ...
                                                    sx={{
                                                        ...
                                                        ...(walletTxFilter === f.key
                                                            ? { bgcolor: f.key === 'debit' ? '#d32f2f' : '#C9A84C', '&:hover': { bgcolor: f.key === 'debit' ? '#b71c1c' : '#085c22' } }
                                                            : { borderColor: f.key === 'debit' ? '#d32f2f' : '#C9A84C', color: f.key === 'debit' ? '#d32f2f' : '#C9A84C' })
                                                    }}>
                                                    {f.label}
                                                </Button>
                                            ))}
                                    ...
                                    {walletTxLoading ? (
                                        ...
                                        <Box sx={{ textAlign: 'center', py: 5, bgcolor: '#F5F5F5', borderRadius: '12px', border: '1px solid #ffcdd2', mb: 4 }}>
                                            <Typography sx={{ color: '#d32f2f', fontWeight: 600, mb: 1 }}>{walletTxError}</Typography>
                                            <Button variant="outlined" size="small" onClick={fetchAllWalletTransactions} sx={{ borderColor: '#C9A84C', color: '#C9A84C', borderRadius: '8px', textTransform: 'none' }}>Try Again</Button>
                                        </Box>
                                    ) : filteredWalletTx.length === 0 ? (
                                        <Box sx={{ textAlign: 'center', py: 5, bgcolor: '#F5F5F5', borderRadius: '12px', border: '1px solid #eee', mb: 4 }}>
                                        ...
                                        <Box sx={{ mb: 4 }}>
                                                        ...
                                                        <TableBody>
                                                            {filteredWalletTx.map((txn, index) => (
                                                                <TableRow key={txn._id || index} hover sx={{ bgcolor: index % 2 === 0 ? '#fcfdfc' : 'white', '&:hover': { bgcolor: '#f4faf5' } }}>
                                                                    <TableCell sx={{ borderBottom: 'none', color: '#CCCCCC', fontSize: '12px' }}>{index + 1}</TableCell>
                                                                    ...
                                                                    <TableCell sx={{ borderBottom: 'none' }}>
                                                                        <Typography sx={{ fontWeight: 800, fontSize: '15px', color: txn.txType === 'credit' ? '#C9A84C' : '#d32f2f' }}>
                                                                    ...
                                                                    <TableCell sx={{ borderBottom: 'none', color: '#CCCCCC', fontSize: '12px', fontFamily: 'monospace' }}>{txn.details || '—'}</TableCell>
                                                                </TableRow>
                                                            ))}
                                                        </TableBody>
                                                    </Table>
                                                </TableContainer>
                                                <Typography variant="caption" sx={{ color: '#CCCCCC', pl: 1 }}>
                                            ...
                                            <Box sx={{ display: { xs: 'flex', md: 'none' }, flexDirection: 'column', gap: 1.5 }}>
                                                {filteredWalletTx.map((txn, index) => (
                                                    <Paper key={txn._id || index} variant="outlined" sx={{ p: 2, borderRadius: '12px', bgcolor: '#F5F5F5', borderLeft: `4px solid ${txn.txType === 'credit' ? '#C9A84C' : '#d32f2f'}` }}>
                                                        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                                                            <Box sx={{ flex: 1 }}>
                                                                ...
                                                                <Typography sx={{ fontSize: '11px', color: '#CCCCCC', mt: 0.25 }}>
                                                                    {new Date(txn.date).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' })}
                                                                </Typography>
                                                            </Box>
                                                            <Typography sx={{ fontWeight: 800, fontSize: '17px', color: txn.txType === 'credit' ? '#C9A84C' : '#d32f2f', flexShrink: 0 }}>
                                                                {txn.txType === 'credit' ? '+' : '-'}₹{txn.amount?.toLocaleString()}
                                                            </Typography>
                                                        </Box>
                                                    </Paper>
                                    ...
                                    {ordersLoading ? (
                                        <Box sx={{ display: 'flex', justifyContent: 'center', py: 4 }}><CircularProgress size={36} sx={{ color: '#C9A84C' }} /></Box>
                                    ) : userOrders.length === 0 ? (
                                        <Box sx={{ textAlign: 'center', py: 4, bgcolor: '#F5F5F5', borderRadius: '12px', border: '1px solid #eee', mb: 4 }}>
                                        ...
                                        <Box sx={{ mb: 4 }}>
                                            {userOrders.map((order, index) => {
                                                ...
                                                const total = order.totalAmount || order.total || '0';
                                                return (
                                                    <Paper key={order._id || index} variant="outlined" sx={{ p: 0, borderRadius: '14px', mb: 2, overflow: 'hidden', cursor: 'pointer', '&:hover': { boxShadow: '0 6px 18px rgba(201,168,76,0.1)', borderColor: '#C9A84C' }, transition: 'all 0.3s ease' }} onClick={() => setTabValue(2)}>
                                                        <Box sx={{ display: 'flex', alignItems: 'center' }}>
                                                            <Box sx={{ p: 2, bgcolor: '#f4faf5', borderRight: '1px solid #f0f0f0', display: 'flex', alignItems: 'center' }}>
                                                                <Box sx={{ width: 44, height: 44, borderRadius: '10px', bgcolor: '#F5F5F5', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                                                                    <ShoppingBagIcon sx={{ color: '#C9A84C', fontSize: 24 }} />
                                                                </Box>
                                                            </Box>
                                                            <Box sx={{ p: 2, flex: 1, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                                                <Box>
                                                                    ...
                                                                    <Typography sx={{ fontSize: '12px', color: '#CCCCCC' }}>{productName} • {new Date(order.createdAt).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' })}</Typography>
                                                                </Box>
                                                                <Box sx={{ textAlign: 'right' }}>
                                                                    <Typography sx={{ fontWeight: 800, color: '#C9A84C', fontSize: '16px' }}>₹{total}</Typography>
                                                                    <Chip label={(order.status || 'Placed').toUpperCase()} size="small" sx={{ height: 20, fontSize: '10px', fontWeight: 800, mt: 0.5, bgcolor: order.status === 'delivered' ? '#C9A84C' : '#C9A84C', color: 'white', borderRadius: '4px' }} />
                                                                </Box>
                                                            </Box>
                                                        </Box>
                                                    </Paper>
                                                    ...
                                                    >
                                                        <Box sx={{ display: 'flex', flexDirection: { xs: 'column', sm: 'row' }, alignItems: 'stretch' }}>
                                                            <Box sx={{
                                                                ...
                                                                <Box sx={{
                                                                    width: 44, height: 44, borderRadius: '10px', bgcolor: '#F5F5F5',
                                                                    display: 'flex', alignItems: 'center', justifyContent: 'center',
                                                                    boxShadow: '0 2px 6px rgba(0,0,0,0.04)'
                                                                }}>
                                                                    <ReceiptIcon sx={{
                                                                        fontSize: 24,
                                                                        color: txn.status === 'success' ? '#C9A84C' : txn.status === 'failed' ? '#d32f2f' : '#fbc02d'
                                                                    }} />
                                                                </Box>
                                                            </Box>
                                                            <Box sx={{ p: 2, flex: 1, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                                                <Box>
                                                                    ...
                                                                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
                                                                        <Typography sx={{ fontSize: '12px', color: '#CCCCCC', display: 'flex', alignItems: 'center', gap: 0.5 }}>
                                                                ...
                                                                <Box sx={{ textAlign: 'right' }}>
                                                                    ...
                                                                    <Chip
                                                                        ...
                                                                        sx={{
                                                                            ...
                                                                            bgcolor: txn.status === 'success' ? '#C9A84C' : txn.status === 'failed' ? '#d32f2f' : '#fbc02d',
                                                                            color: 'white',
                                                                            borderRadius: '4px'
                                                                        }}
                                                                    />
                                    ...
                                    {userTransactions.length === 0 && (
                                        <Box sx={{ textAlign: 'center', py: 4, bgcolor: '#F5F5F5', borderRadius: '12px', border: '1px solid #eee' }}>
                                            ...
                                            <Button variant="outlined" size="small" sx={{ mt: 2, borderColor: '#C9A84C', color: '#C9A84C', borderRadius: '8px' }} onClick={() => navigate('/recharge')}>
                                                Make a Payment
                                            </Button>
                                        </Box>
                                    )}
                            ...
                            {tabValue === 4 && (
                                <Box>
                                    ...
                                    {grievancesLoading ? (
                                        <Box sx={{ display: 'flex', justifyContent: 'center', py: 4 }}><CircularProgress size={40} sx={{ color: '#C9A84C' }} /></Box>
                                        ...
                                        <Box sx={{ textAlign: 'center', py: 4 }}>
                                            ...
                                            <Button variant="contained" startIcon={<AssignmentIcon />} sx={{ bgcolor: '#C9A84C', '&:hover': { bgcolor: '#e67e22' } }} onClick={() => navigate('/grievance')}>Submit a Grievance</Button>
                                        </Box>
                                    ) : (
                                            ...
                                            {userGrievances.map((grievance, index) => (
                                                <Grid item xs={12} key={index}>
                                                    <Zoom in={showContent} timeout={600} style={{ transitionDelay: `${index * 100}ms` }}>
                                                        <Card variant="outlined" sx={{ '&:hover': { boxShadow: '0 8px 24px rgba(10,122,47,0.12)', borderColor: '#C9A84C' }, transition: 'all 0.3s ease', borderRadius: '14px', overflow: 'hidden' }}>
                                                            <Box sx={{ display: 'flex', flexDirection: { xs: 'column', md: 'row' }, p: 0 }}>
                                                                <Box sx={{ p: 3, flex: 1.5 }}>
                                                                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, mb: 1.5 }}>
                                                                        <Typography variant="caption" sx={{ color: '#C9A84C', fontWeight: 800, display: 'flex', alignItems: 'center', gap: 0.75, bgcolor: '#e8f5e9', px: 1.5, py: 0.5, borderRadius: '6px' }}>
                                                                ...
                                                                <Box sx={{ width: { xs: '100%', md: '200px' }, bgcolor: '#1A1A1A', borderLeft: { xs: 'none', md: '1px solid #eaeaea' }, borderTop: { xs: '1px solid #eaeaea', md: 'none' }, p: { xs: 2.5, sm: 3 }, display: 'flex', flexDirection: { xs: 'row', md: 'column' }, justifyContent: 'center', alignItems: 'center', gap: 2 }}>
                                                                    ...
                                                                    <Button fullWidth variant="contained" startIcon={<HistoryIcon />}
                                                                        sx={{ bgcolor: '#C9A84C', '&:hover': { bgcolor: '#086325' }, textTransform: 'none', borderRadius: '8px', py: 1.2, fontWeight: 700, boxShadow: 'none', flex: { xs: 1, md: 'none' } }}
                ...
                <Snackbar open={snackbar.open} autoHideDuration={3000} onClose={() => setSnackbar({ ...snackbar, open: false })} anchorOrigin={{ vertical: 'bottom', horizontal: 'right' }} TransitionComponent={Slide}>
                    <Alert onClose={() => setSnackbar({ ...snackbar, open: false })} severity={snackbar.severity} variant="filled"
                        sx={{ width: '100%', borderRadius: '16px', fontWeight: 800, bgcolor: '#C9A84C', color: 'white', '& .MuiAlert-icon': { color: 'white' } }}>
                        {snackbar.message}
                    </Alert>
                </Snackbar>
