...
const getRankColor = (rank) => {
    const colors = {
        ...
        "Emerald": "#10b981", "Diamond": "#C9A84C", "MD": "#FFD700",
    };
...
const UserDashboardLayout = () => {
    ...
    const menuItems = [
        ...
        { id: 'orders', name: 'Product Order', icon: ShoppingCart, path: '/my-account/orders', badge: stats?.totalOrders || '0', badgeColor: 'bg-[#C9A84C]' },
        {
            id: 'first_bonus',
    ...
    return (
        <div className="min-h-screen bg-zinc-900 flex">
            {/* Sidebar */}
            <aside
                className={`fixed left-0 h-[calc(100vh-60px)] md:h-[calc(100vh-80px)] top-[60px] md:top-[80px] bg-[#C9A84C] text-zinc-100 transition-all duration-300 z-50 shadow-none overflow-y-auto no-scrollbar
                        ...
                        <div className={`flex items-center ${sidebarOpen ? 'space-x-3' : 'flex-col space-y-4'}`}>
                            <button
                                ...
                                className={`w-12 h-12 rounded-2xl flex items-center justify-center shrink-0 transition-all active:scale-95
                                    ${sidebarOpen ? 'bg-zinc-900/5 hover:bg-zinc-900/10' : 'bg-zinc-900/20 hover:bg-zinc-900/30'}`}
                                title={sidebarOpen ? "Toggle Menu" : "Open Menu"}
                            >
                                {sidebarOpen ? <Shield className="w-6 h-6 text-zinc-100" /> : <Menu className="w-6 h-6 text-zinc-100" />}
                            </button>
                            {sidebarOpen && (
                                <Link to="/" className="leading-none text-zinc-100 hover:opacity-80 transition-opacity flex flex-col pt-1">
                                    <h2 className="font-black text-2xl whitespace-nowrap uppercase tracking-tighter">SANYUKT</h2>
                                    <p className="text-[10px] text-zinc-100/80 uppercase tracking-[0.4em] font-black -mt-0.5">Parivaar</p>
                                </Link>
                            )}
                        </div>
                    </div>
                    ...
                    {sidebarOpen && (
                        <div className="mb-6 px-6 flex items-center gap-4">
                            <div className="relative shrink-0">
                                ...
                                <div className="absolute bottom-0 right-0 w-3.5 h-3.5 bg-yellow-500 border-2 border-[#C9A84C] rounded-full"></div>
                            </div>
                            <div className="overflow-hidden flex-1">
                                <h3 className="font-black text-sm uppercase tracking-wide text-zinc-100 truncate">{userData.userName}</h3>
                                <p className="text-[10px] text-zinc-100/70 font-bold uppercase tracking-widest">{userData.memberId || 'SPRL0000'}</p>
                                ...
                                <div className="flex items-center gap-2 text-zinc-100/90 mt-1">
                                ...
                                <div key={item.id} className="w-full">
                                    <button
                                        ...
                                        className={`relative w-full flex items-center transition-all duration-300 group border-b border-[#096628]
                                            ...
                                            ${active
                                                ? 'bg-[#096628] text-zinc-100 shadow-inner'
                                                : 'text-zinc-100 hover:bg-[#096628]'}`}
                                    >
                                        <item.icon className={`${sidebarOpen ? 'w-7 h-7 mr-4' : 'w-7 h-7'} text-zinc-100 shrink-0 transition-transform duration-300 group-hover:scale-110`} strokeWidth={2.5} />
                                            ...
                                            <div className="flex items-center justify-between flex-1 overflow-hidden">
                                                ...
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    {item.badge !== undefined && item.badge !== null && (
                                                        <span className={`px-1.5 py-0.5 rounded text-[11px] font-black text-zinc-100 ${item.badgeColor || 'bg-zinc-900/20'}`}>
                                                            {item.badge}
                                                        </span>
                                                    )}
                                                    {hasChildren && (
                                                        <ChevronDown
                                                            size={18}
                                                            className={`text-zinc-100/70 transition-transform duration-300 ${openDropdown === item.id ? 'rotate-180' : ''}`}
                                                        />
                                                    )}
                                                </div>
                                            </div>
                                        ...
                                        {!sidebarOpen && (
                                            <div className="absolute left-full ml-4 px-3 py-2 bg-slate-900 text-zinc-100 text-[11px] font-bold rounded-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all whitespace-nowrap z-[100] shadow-xl border border-white/10 pointer-events-none">
                                                {item.name}
                                                {item.badge && <span className="ml-2 bg-yellow-500 px-1 rounded text-[9px]">{item.badge}</span>}
                                                {item.rankBadge && <span className="ml-2 bg-zinc-900/20 px-1 rounded text-[9px]">{item.rankBadge}</span>}
                                            </div>
                                        )}
                                    </button>
                                                ...
                                                return (
                                                    <Link
                                                        ...
                                                        className={`block pl-12 pr-4 py-2 text-[13px] transition-all rounded-xl mx-2 ${childActive
                                                            ...
                                                            : 'text-zinc-100/80 hover:text-zinc-100 hover:bg-zinc-900/5 font-bold'
                                                            }`}
                                                    >
                                                        {child.name}
                                                    </Link>
                        ...
                        <div className="w-full">
                            <button
                                ...
                                className={`relative w-full flex items-center transition-all duration-300 group
                                    ...
                                    text-zinc-100/70 hover:text-zinc-100 hover:bg-zinc-900/5`}
                            >
            ...
            >
                <div className="flex-1 px-4 md:px-8 pb-8 pt-0 animate-fadeIn relative">
                    {!sidebarOpen && isMobile && (
                        <div className="py-5 mb-4 flex items-center space-x-4 text-[#C9A84C] md:hidden">
                            <button
                                onClick={() => setSidebarOpen(true)}
                                className="w-14 h-14 flex items-center justify-center bg-zinc-900 shadow-xl shadow-black/5 border border-yellow-900/30 rounded-[2rem] active:scale-95 transition-all text-[#C9A84C]"
                            >
                                <Menu size={28} />
                            </button>
                            <div className="flex flex-col leading-none">
                                <span className="font-black text-2xl tracking-tighter uppercase">Menu</span>
                                <span className="text-[10px] font-black text-zinc-300 uppercase tracking-widest">Navigation</span>
                            </div>
                        </div>
                    )}
                    <Outlet />
            ...
            {isMobile && sidebarOpen && (
                <div
                    className="fixed inset-0 bg-[#C9A84C]/20 backdrop-blur-sm z-40"
